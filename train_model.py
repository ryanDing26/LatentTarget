...
# possibly force single-gpu usage
...
parser.add_argument('--dataset', type=str, default='cd2020',
                    help='cd2020')
parser.add_argument('--datadir', type=str, default='cd2020/',
                    help='qm9 directory')
...
parser.add_argument("--conditioning", nargs='+', default=[],
                    help='arguments : homo | lumo | alpha | gap | mu | Cv | pocket' )
...
parser.add_argument('--include_charges', type=eval, default=True,
                    help='include atom charge or not') # usually not included
...
dataset_info = get_dataset_info(args.dataset, True) # second arg doesn't matter; will phase out later
...
# will be used for pocket conditioning
...
def main():
    ...
    # Initialize dataparallel if enabled and possible. Typically on
    ...
    # Author: This is typically on at value 0.999
    if args.ema_decay > 0:
        model_ema = copy.deepcopy(model)
        ema = flow_utils.EMA(args.ema_decay)
        # This is also typically on for GPU training
        if args.dp and torch.cuda.device_count() > 1:
    ...
    for epoch in range(args.start_epoch, args.n_epochs):
        ...
        if epoch % args.test_epochs == 0:
            ...
            # calculating validation and testing loss
            ...
            # If model performs better than current, save
            if nll_val < best_nll_val:
                best_nll_val = nll_val
                best_nll_test = nll_test
                # Model typically saved
                if args.save_model:
                    args.current_epoch = epoch + 1
                    utils.save_model(optim, 'outputs/%s/optim.npy' % args.exp_name)
                    utils.save_model(model, 'outputs/%s/generative_model.npy' % args.exp_name)
                    # usually goes into this block
                    if args.ema_decay > 0:
