...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    def __init__(
            self,
            dynamics: models.EGNNDynamics, in_node_nf: int, n_dims: int,
...
class EnHierarchicalVAE(torch.nn.Module):
    ...
    def __init__(
            self,
            encoder: models.EGNNEncoder,
            decoder: models.EGNNDecoder,
            in_node_nf: int, n_dims: int, latent_node_nf: int,
            kl_weight: float, structure,
    ...
    def encode(self, x, h, node_mask=None, edge_mask=None, context=None):
        ...
        # For proteins, re-center if needed
        if self.structure == 'protein' and node_mask is not None:
            for b in range(x.shape[0]):
                valid_mask = node_mask[b].squeeze() > 0.5
                if valid_mask.sum() > 0:  # Check we have valid atoms
                    # Re-center the valid atoms
                    valid_coords = x[b][valid_mask]
                    com = valid_coords.mean(dim=0)
                    if com.abs().max() > 1e-3:  # Only re-center if needed
                        x[b][valid_mask] = valid_coords - com
                        # Update xh with centered coordinates
                        xh[b, :, :self.n_dims] = x[b]
    ...
    def decode(self, z_xh, node_mask=None, edge_mask=None, context=None):
        ...
        # For proteins, re-center if needed (same approach as encode)
        if self.structure == "protein" and node_mask is not None:
            for b in range(x_recon.shape[0]):
                valid_mask = node_mask[b].squeeze() > 0.5
                if valid_mask.sum() > 0:  # Check we have valid atoms
                    # Re-center the valid atoms
                    valid_coords = x_recon[b][valid_mask]
                    com = valid_coords.mean(dim=0)
                    if com.abs().max() > 1e-3:  # Only re-center if needed (same threshold as encode)
                        x_recon[b][valid_mask] = valid_coords - com
...
class EnLatentDiffusion(EnVariationalDiffusion):
    ...
    def forward(self, x, h, node_mask=None, edge_mask=None, context=None):
        ...
        # Only assert zero mean for proteins, not ligands
        # if hasattr(self.vae, 'structure') and self.vae.structure == 'protein':
