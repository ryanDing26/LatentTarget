...
# keep; use for rdkit metrics
from qm9.analyze import analyze_stability_for_molecules
# keep ig
from qm9.sampling import sample_chain, sample, sample_sweep_conditional
# keep
import utils
# only used to prepare context; may not be needed later
...
# Method is done and ready for preprocesing
# from torch_scatter import scatter_mean
# def center_complexes(protein_pos, ligand_pos):
#     """Preprocess all protein-ligand complexes via adjustings coords to protein CoM
    
#     Parameters
#     ----------
#     protein_pos: torch.Tensor
#         tensors of protein coordinates
#     ligand_pos: torch.Tensor
#         tensors of associated ligand coordinates

#     Returns
#     -------
#     protein_pos, ligand_pos: torch.Tensor, torch.Tensor
#         Adjusted coordinates based on protein CoM
#     """
#     batches = torch.arange(len(protein_pos))
#     protein_centroids = scatter_mean(protein_pos, batches, dim=0)
#     protein_pos -= protein_centroids
#     ligand_pos -= protein_centroids
...
def train_epoch(args, loader, epoch, model, model_dp, model_ema, ema, device, dtype, property_norms, optim,
    ...
    for i, data in enumerate(loader):
        ...
        # conditional QM9 and GEOM models trained without, unconditional QM9 trained with; likely just going to do no charges for CD
        charges = (data['charges'] if args.include_charges else torch.zeros(0)).to(device, dtype)

        x = remove_mean_with_mask(x, node_mask)

        # configs have it such that you don't augment noise
        ...
        # regularization term is actually 0
        loss = nll + args.ode_regularization * reg_term
        
        # backprop
        loss.backward()

        # typically gradient clipping
        grad_norm = utils.gradient_clipping(model, gradnorm_queue) if args.clip_grad else 0

        optim.step()

        # Update EMA if enabled. Usually is.
        if args.ema_decay > 0:
...
def check_mask_correct(variables, node_mask):
    for variable in variables:
        if len(variable) > 0: assert_correctly_masked(variable, node_mask)
...
def test(args, loader, epoch, eval_model, device, dtype, property_norms, nodes_dist, partition='Test'):
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(loader):
            ...
            # typically zeroed in conditional generation
            charges = (data['charges'] if args.include_charges else torch.zeros(0)).to(device, dtype)
